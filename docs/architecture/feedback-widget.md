...
| Widget modes | Embed panel is **Loki-on-the-site**: `data-fc-modes` picks Chat (default) / Report / Watch (`widget/surface-modes.ts`; unshipped modes render no control). **Chat** talks to Loki (execution), Cat (economy, OrangeCat) or Solon (governance) — agents in `widget/agents.ts`, personas in `src/config/widget-agents.ts`, served by `/api/widget/chat` (fcw_* token + origin allowlist, stateless, streamed). "Send to Loki to build" hands the conversation to `/api/feedback` as a brief; Cat cannot move money and hands off to `/dashboard/cat` on OrangeCat; Solon cannot vote and hands off to a pre-filled Solon `/propose` (title + body from the chat). Handoff URLs arrive with boot (`chat.handoffs`). Watch is still a seam |

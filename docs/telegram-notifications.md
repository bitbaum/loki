...
| 🗓 Worth your week — up to six events, each with a one-tap ✅ that puts it in the calendar, and ⚠️ when it clashes with something already booked | Weekly scout run: searches for Zürich/Swiss meetups, conferences, hackathons, civic and cultural events, extracts them in ONE batched model call, drops anything undated or outside three weeks, and skips whatever was already proposed or rejected. Sends NOTHING on a quiet week, and nothing when no search backend answered (logged instead — a silent digest must never read as "nothing is on") | `src/app/api/crons/scout-events/route.ts` |

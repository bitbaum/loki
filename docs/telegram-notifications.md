...
| 🌐 \<project\> is live (the site URL, plus the owner's link that readies the widget so what they say there gets built) | A project with a repository but no live URL is found live — a green deploy of main AND a public response — by the hourly `check-site-live` cron or when the owner opens the project page. Once per project: the live URL it writes is what stops the check. Operator's own projects only; other owners get the same as a push | `src/lib/site-live-reconcile.ts` |
...
| 🧪 loki e2e FAILED: … / ✅ RECOVERED | End-to-end check of the live site as the operator, ON DEMAND only since 2026-09-25 (no timer: it sends a real chat turn, and a timer may not spend the shared free AI tier): fleet question answered from the map, map published, dispatch returns to its thread (when the builder is authenticated) | `scripts/hetzner/loki-e2e-check.sh` (installed by `scripts/hetzner/install-loki-e2e-watch.sh`; test: `scripts/test/loki-loop-e2e.ts`) |
